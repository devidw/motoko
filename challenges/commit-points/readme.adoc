...
When we code actors we handle messanging in async shared functions, that handle
messanges.

The actor itself has a isolated state, which we can change only from inside
these functions.

However, changing the state in these functions is not commiting these state
changes directly to the actor state.

Instead there are a couple of differnt commit points, which when reached, will
...
We we close our function by returning a result, this is a commit point that
